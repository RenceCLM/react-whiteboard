...
//     // ✅ Draw selection box & resize handles
//     console.log("Drawing selection box", selectedElements);
//     if (selectedElements.length > 0) {
//       const box = getBoundingBox(selectedElements);
//       if (box) {
//         selectedBoxRef.current = box;
...
//         ...activeInputRef.current, // Access the current value
//         ...clickedTextBox,
//         text: clickedTextBox.text,
//       }
//       return;
...
//         activeInputRef.current = {...activeInputRef.current,
//           x: startTextBox.x,
//           y: startTextBox.y,
//           width: width,
//           height: height,
...
//     const lines = textarea.value.split("\n");
//     const maxLineWidth = Math.max(...lines.map((line) => ctx.measureText(line).width));
//     const newWidth = Math.max(activeInputRef.current.width, maxLineWidth / scaleRef.current + padding * 2);
...
//     // Find and update the textbox in textBoxesRef
//     textBoxesRef.current = textBoxesRef.current.map((textbox) =>
//       textbox.id === activeInputRef.current.id
//         ? { ...textbox, text: " ",  height: newHeight, width: newWidth } // Update the textbox with new dimensions, also text " " to remove flavor text
//         : textbox
//     );
...
//       // ✅ Save the input value back into the textbox
//       const updatedBoxes = textBoxesRef.current.map((box) =>
//         box.id === activeInputRef.current.id
//           ? { ...box, text: activeInputRef.current.text }
//           : box
//       );
//       textBoxesRef.current = updatedBoxes;

//       const updatedNotes = notesRef.current.map((note) =>
//         note.id === activeInputRef.current.id
//           ? { ...note, text: activeInputRef.current.text }
//           : note
//       );
//       notesRef.current = updatedNotes;
...
//        {activeInputRef.current && (
//         <textarea
//           value={activeInputRef.current.text}
//           onChange={handleInputChange}
//           style={{
//             position: "absolute",
//             left: `${activeInputRef.current.x * scaleRef.current + translationRef.current.x + canvasRef.current.getBoundingClientRect().left + borderWidth}px`,
//             top: `${activeInputRef.current.y * scaleRef.current + translationRef.current.y + canvasRef.current.getBoundingClientRect().top + borderWidth}px`,
//             width: `${activeInputRef.current.width * scaleRef.current}px`,
//             height: `${activeInputRef.current.height * scaleRef.current}px`,
//             font: "16px Arial",
//             padding: "4px",
//             border: "1px solid blue",
//             zIndex: 10,
